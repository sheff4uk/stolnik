			...
			if( mysqli_num_rows($res) ) {
				while( $row = mysqli_fetch_array($res) ) {
					...
					while( $subrow = mysqli_fetch_array($subres) ) {
						...
						echo ($subrow["code"] ? "<img style='width: 100px; float: left; margin-right: 20px; margin: 0 20px 20px 0; border-radius: 10px; box-shadow: 5px 5px 15px #333; filter: grayscale(1);' src='/images/prodlist/{$subrow["code"]}.jpg'/>" : "");
						?>
						<script>
							$( function() {
